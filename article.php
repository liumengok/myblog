		 ...
		 ?> 
          ...
          <tr> 
			...
			if ($page){
			   ...
			   $query="select count(*) as total from comment where articleId='  ,m ,' order by id desc";   
			   ...
			   for ($i=1; $i<2; $i++) {         //计算每页显示几行记录信息
				if ($i==1) {
					$sql=mysql_query("select * from comment where articleId='$file_id1' order by id desc limit $offset, $page_size");		
					$result=mysql_fetch_array($sql);
				}
				if($result==false){
		...
		<form name="myform" method="post" action="check_comment.php">
 		   <table width="660" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="#9CC739" bgcolor="#FFFFFF">
 		      ...
 		      <td height="112" align="center" valign="top" >
				    ...
				    <tr align="center">
					  <td colspan="2"><input type="submit" name="submit" value="提交" onClick="return r_check(myform);">
					    &nbsp;
